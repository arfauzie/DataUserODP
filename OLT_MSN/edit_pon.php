...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($pon)) {
    ...
    if (!empty($perubahan)) {
        $stmt = $pdo->prepare("UPDATE pon1 SET nama_pon = ?, port_max = ? WHERE id = ?");
        $stmt->execute([$nama_pon, $port_max, $id]);

        // Ambil nama admin
        $oleh = is_array($_SESSION['admin']) ? ($_SESSION['admin']['username'] ?? 'admin') : $_SESSION['admin'];

        // Simpan log menggunakan $pdo sebagai parameter pertama
        $log_keterangan = implode(" | ", $perubahan);
        tambahRiwayat($pdo, "Edit PON", $oleh, $log_keterangan);
