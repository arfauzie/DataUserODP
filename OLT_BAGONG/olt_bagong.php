...
if (isset($_POST['tambah_pon'])) {
    ...
    $stmt = $pdo2->prepare("INSERT INTO $pon_table (olt_id, nama_pon, port_max) VALUES (?, ?, ?)");
    if ($stmt->execute([$olt_id, $nama_pon, $port_max])) {
        $last_id = $pdo2->lastInsertId(); // Ambil ID PON yang baru
...
if (isset($_POST['tambah_odp'])) {
    ...
    $stmt = $pdo2->prepare("SELECT port_max, (SELECT COUNT(*) FROM $odp_table WHERE pon_id = ?) as jumlah_odp FROM $pon_table WHERE id = ?");
    $stmt->execute([$pon_id, $pon_id]);
    $result = $stmt->fetch();

    if ($result && $result['jumlah_odp'] < $result['port_max']) {
        $stmt = $pdo2->prepare("INSERT INTO $odp_table (pon_id, nama_odp, port_max) VALUES (?, ?, ?)");
        ...
        echo "<script>alert('Gagal! Jumlah ODP sudah mencapai batas maksimum.'); window.location='olt_bagong.php?pon_id={$pon_id}';</script>";
    }
}
        ...
        <div class="content">
            ...
            ?>
                ...
                if (isset($_POST['cek_odp_terdekat'])) {
                    ...
                    $stmt = $pdo2->query("SELECT o.id, o.nama_odp, o.latitude, o.longitude, p.nama_pon,
        (6371 * ACOS(
            ...
            ?>
                <div class="card-box">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="olt_bagong.php" class="btn btn-secondary btn-lg">Kembali</a>
                    </div>
            ...
            ?>
                <div class="card-box">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="olt_bagong.php?pon_id=<?= $pon_id ?>" class="btn btn-secondary btn-lg">Kembali</a>
                    </div>
