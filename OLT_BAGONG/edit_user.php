...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_odp') {
    ...
    $stmt = $pdo2->prepare("SELECT id, nama_odp FROM odp2 WHERE pon_id = ? ORDER BY nama_odp ASC");
    $stmt->execute([$pon_id]);
...
if ($pon_id <= 0 && $odp_id > 0) {
    $stmt = $pdo2->prepare("SELECT pon_id FROM odp2 WHERE id = ?");
    $stmt->execute([$odp_id]);
...
$all_odps = [];
if ($pon_id > 0) {
    $odp_stmt = $pdo2->prepare("SELECT id, nama_odp FROM odp2 WHERE pon_id = ? ORDER BY nama_odp ASC");
        ...
        if (!$target || (int)$target['pon_id'] !== $pon_id_new) {
            ...
            if ($pindahOdp) {
                $cek = $pdo2->prepare("
                    ...
                    FROM odp2 o
                    LEFT JOIN users2 u ON u.odp_id = o.id
                    WHERE o.id = ?
        ...
        <div class="card-box">
            ...
            <form method="POST">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <a href="olt_bagong.php?pon_id=<?= (int)$pon_id ?>&odp_id=<?= (int)$odp_id ?>" class="btn btn-secondary">Kembali</a>
                </div>
            </form>
        </div>
    </div>
