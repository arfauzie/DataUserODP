...
if (isset($_POST['tambah_pon'])) {
    ...
    if ($stmt->execute([$olt_id, $nama_pon, $port_max])) {
        $last_id = $pdo3->lastInsertId(); // Ambil ID PON baru
        // Ambil nama OLT (supaya log lebih jelas)
        $stmtOlt = $pdo3->prepare("SELECT nama_olt FROM olt WHERE id = ?");
    ...
    <div class="container mt-4">
        <div class="content">
            ...
            SELECT 
                ...
                o.port_max,
                p.nama_pon,
                (SELECT COUNT(*) FROM users3 u WHERE u.odp_id = o.id) AS jumlah_user,
                (6371 * ACOS(
        ...
        ");
                    ...
                    if ($closest) {
                        ...
                        $link_odp     = "olt_soreang.php?pon_id=" . urlencode($closest['pon_id']) . "&odp_id=" . urlencode($closest['odp_id']);
